    ...
    if child.spelling.find('of') == 0:
        ...
        if utils.is_function(child):
            if child.spelling not in visited_function and offilename != "ofMain":
                ...
                add_function(new_func.serialize(), offilename)
...
def parse_function(documentation_class, clazz, member, already_found, fuzzy=False):
        ...
        for part in arg.get_children():
            if part.kind == CursorKind.INTEGER_LITERAL or \
                ...
                try:
                    ...
                    print "error trying to print default value for " + documentation_class.name + "::" + member.spelling + " " + arg.spelling + " = " + str(
                        part.kind)
                    pass
    ...
    # Returns
    if member.kind == CursorKind.CONSTRUCTOR or member.kind == CursorKind.DESTRUCTOR or (
        not clazz is None and methodname == clazz.spelling):
        returns = ""
    else:
...
def serialize_class(cursor, is_addon=False, parent=None):
    ...
    documentation_class.path = os.path.dirname(clazz.location.file.name).replace(
        os.path.join(of_root, "libs/openFrameworks/"), "")
